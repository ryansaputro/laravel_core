...
<style>
	...
	@endif
<form action="{{ route('barang_vendor.update',['id' => $id]) }}" class="myform" method="post" enctype="multipart/form-data">
	@csrf
	...
	<tbody>
		<tr class="tr_clone" data-id="1">
			...
			<select data-live-search="true" class="form-control" id="id_barang" required name="id_barang[]">
				...
				<option value="{{$v->id_barang}}" {{$barangSelected->id_barang == $v->id_barang ? 'selected' : ''}}>{{$v->nama_barang}} ({{$v->kode_barang}})</option>
				@endforeach
			</select>
			</td>
			<td>
				<select data-live-search="true" class="form-control" id="id_vendor" required name="id_vendor[]">
					...
					<option value="{{$v->id_vendor}}" {{$barangSelected->id_vendor == $v->id_vendor ? 'selected' : ''}}>{{$v->nama_vendor}}</option>
					@endforeach
				</select>
			</td>
			<td>
				<select data-live-search="true" class="form-control" id="id_satuan" required name="id_satuan[]">
					...
					<option value="{{$v->id_satuan}}" {{$barangSelected->id_satuan == $v->id_satuan ? 'selected' : ''}}>{{$v->nama_satuan}}({{$v->kode_satuan}})</option>
					@endforeach
				</select>
			</td>
			<td>
				<input type="number" value='{{$barangSelected->qty}}' class="form-control" id="qty" required name="qty[]">
			</td>
			<td>
				<input type="number" value='{{$barangSelected->harga_beli}}' class="form-control" id="harga_beli" required name="harga_beli[]">
			</td>
			<td>
				<input type="number" value='{{$barangSelected->harga_jual}}' class="form-control" id="harga_jual" required name="harga_jual[]">
			</td>
			<td>
