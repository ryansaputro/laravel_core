...
@section('content')
    ...
    @endif
	{!! Form::model($menus, ['method' => 'PATCH','route' => ['menus.update', $menus->id]]) !!}
	<div class="row">
                ...
                <select name="icon" data-show-content="true"  class="form-control" id="mySelect">
                    <option disabled selected>-pilih-</option>
                      @foreach ($icons as $k => $v)
                        <option data-content="<i class='{{$v}}'></i>" {{$menus->icon == $v ? 'selected' : ''}}>{{$v}}</option>
                    @endforeach
                </select>
            </div>
        </div>
                ...
                <select name="menu_role" class="form-control">
                    <option disabled selected>-pilih-</option>
                      @foreach ($menuRoles as $k => $v)
                        <option value="{{$v->id}}" {{$menus->menu_role == $v->id ? 'selected' : ''}} >{{$v->name}}</option>
                    @endforeach
                </select>
            </div>
        </div>
                ...
                <select name="url" data-show-content="true"  class="form-control" id="mySelect">
                    <option disabled selected>-pilih-</option>
                      @foreach ($pages as $k => $v)
                        <option value="{{$v->url}}" {{$menus->url == $v->url ? 'selected' : ''}}>{{$v->url}}</option>
                    @endforeach
                </select>
            </div>
        </div>
        <div class="col-xs-12 col-sm-12 col-md-12">
            <div class="form-group">
                <strong>Status:</strong>
                    <label>{{ Form::radio('status', 1, ($menus->status == '1') ? true : false, array('class' => 'status')) }}
                        Aktif 
                    </label>
                    <label>{{ Form::radio('status', 0, ($menus->status == '0') ? true : false, array('class' => 'status')) }}
                       Non Aktif 
                    </label>
            </div>
        </div>
